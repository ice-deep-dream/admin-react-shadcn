...
export function TasksMultiDeleteDialog<TData>({
  ...
  const handleDelete = () => {
    ...
    toast.promise(sleep(2000), {
      loading: t('common.deletingTasks'),
      success: () => {
        setValue('')
        table.resetRowSelection()
        return t('common.deletedXTasks', {
          count: selectedRows.length,
          task: selectedRows.length > 1 ? t('common.tasks') : t('common.task')
        })
      },
      ...
      title={
        <span className='text-destructive'>
          <AlertTriangle
            ...
            size={18}
          />{' '}
          {t('common.deleteXTasks', { count: selectedRows.length })}
        </span>
      }
      desc={
        <div className='space-y-4'>
          <p className='mb-2'>
            {t('common.areYouSureYouWantToDeleteTheSelectedTasks')} <br />
